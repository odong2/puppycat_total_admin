...
public class S3Library {
    ...
    private final List<String> IMAGE_EXTENSION_LIST = Arrays.asList("jpg", "jpeg", "png", "json");
    @Value("${env.server}")
    ...
    private String memberCfImgURL;
    @Value("${cloud.aws.s3.img.url.sns}")
    private String snsCfImgURL;
    @Value("${cloud.aws.s3.img.url.common}")
    private String commonCfImgURL;
    @Value("${cloud.aws.s3.img.url.shop}")
    private String shopCfImgURL;
    @Autowired
    private TelegramLibrary telegramLibrary;

    public String bucketChange(String changeBucket) {
        bucket = changeBucket;
        return bucket;
    }
     ...
     */
    public String getUploadedFileName(String fileUrl, String imgType) {
        try {
            String cfImgURL = "";

            if (imgType.equals("sns")) {
                cfImgURL = snsCfImgURL;
            } else if (imgType.equals("member")) {
                cfImgURL = memberCfImgURL;
            } else if (imgType.equals("common")) {
                cfImgURL = commonCfImgURL;
            } else if (imgType.equals("shop")) {
                cfImgURL = shopCfImgURL;
            } else {
                cfImgURL = snsCfImgURL;
            }
    ...
    public boolean upload(String bucket, File file, String key) {
        return uploadToS3(new PutObjectRequest(bucket, key, file));
    }

    private boolean uploadToS3(PutObjectRequest putObjectRequest) {
